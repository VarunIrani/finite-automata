...
class SimulationModal extends Component {
  ...
  validate(machineType, alphabets) {
    ...
    if (machineType === MachineType.DFA) {
      machineData.machine_name = "Test DFA"
      ...
      if (states.length === 0) {
        ...
        states.forEach(s => {
          ...
          s.transitions.forEach(t => {
            ...
            if (value.includes(',') && alphabets.every(a => value.includes(a))) {
              ...
              if (alphabets.some(a => value.includes(a)))
                count += 1;
            }
          })
        })
        ...
        if (finalStateCount === 0)
          errors.push("There are no final states. Please add at least one final state.")
    ...
    if (errors.length === 0)
      this.setState({errors, valid: true, machineData})
    else
    ...
    return (
      <Modal.Dialog style={{zIndex: 9999}} hidden={this.props.show}>
        ...
        <Modal.Footer>
          ...
          <Button variant="danger" ref={(node) => {
            this.close = node
          }} onClick={() => {
            this.setState({
              valid: false,
